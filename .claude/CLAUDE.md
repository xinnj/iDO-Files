...
**`tests/env/nginx.conf` is a parallel copy of the root `nginx.conf`, not an include** — they differ by ~430 lines (paths, `worker_processes`, `lua_package_path`, log format). A change to the root config is invisible to the suite until it is mirrored there, so config changes are made twice; the startup check above had to go in both. `env/start.sh` also re-runs `seed-data.sh` on every invocation, which **overwrites `tests/env/data/config/auth_config.json`** — a hand-written bad config will be silently replaced before nginx reads it, so probing the startup check means stopping nginx, writing the file, and launching `openresty -p tests/env -c tests/env/nginx.conf` directly rather than going through `start.sh`.
...
**The same invariant on the *read* path is what caused the ci outage.** `authorize.lua`'s `read_config()` rejects the whole file if any group lacks `allow` or `deny` — the same rule as `validate_config`, enforced independently. The difference is *when* it fires: `load_config()` reads the file only on the first request after the shared dict goes cold, which is exactly what a restart produces. So an invalid `auth_config.json` does not fail at startup; it fails on the first request *after* a restart and denies **every** request, and because `save_config_to_redis` sets `initialized` only on success, every later request retries and fails identically. The outage is total and self-perpetuating, the file can sit invalid for months while Redis stays warm and everything looks fine, and the only trace is a per-request error log. `nginx.conf`'s `init_by_lua_block` now calls `authorize.validate_config_file()` and refuses to start, so it is a crashloop that names the problem instead of a delayed blackout. Two details there are load-bearing: the error names **every** broken group at once (`pairs()` order over the rules table is unspecified, so an early return names an arbitrary one and costs a restart per mistake), and `read_config` guards `config.rules` itself because `pairs(nil)` raises — which surfaced as a Lua traceback rather than a reason.

Verified by launching nginx against a config with `.default: {}` and a `deny`-less `fileserver_admin`: `exit=1`, no listener, and `auth_config.json is invalid: group 'fileserver_admin' is missing 'deny'; group '.default' is missing 'allow' and 'deny'`.

**Where that `.default: {}` came from — the save path wrote it.** `auth-config.lua` serialised with `for_encoding()`, which swapped each empty rule list for `cjson.empty_array`. That sentinel is an **OpenResty-fork extension, and the image is not OpenResty**: `build-base.sh` builds `nginx:1.27.1-alpine` with `ENABLED_MODULES="ndk lua"`, and the app image `apk add`s Alpine's `lua5.1-cjson` **2.1.0** — which has no `empty_array`, no `empty_array_mt`, no `encode_empty_table_as_object`, and ignores `__jsontype`. Assigning the missing sentinel does not raise: `t[key] = nil` **removes the key**, so every role with an empty list silently lost that list and a role whose `allow` and `deny` were *both* empty was written as the bare `{}` the startup check then refuses. Hence an admin who edits only *another* role can fail the next restart on `.default`, a role they never touched — the POST carries the whole config. `validate_config` cannot catch it (it validates the *incoming* rules, and the file is written from the *encoded* output) and the unit tests cannot either, because they run against whatever cjson is on the machine and both OpenResty's and homebrew's forks have the sentinel — so it only ever failed in production. `lua/json.lua` now emits the array literal itself (`encode_array`, `encode_object`) so `[]` survives any cjson; `handler.lua`'s embedded `files` and `respond_roles` had the same bug, where an empty folder emitted no `files` key at all and `app.js` spreads it. `auth-config_spec.lua` has a block that nils `cjson.empty_array` for its duration, which is the only way the suite can see this.
...
**Admin pages** load only `bootstrap.min.css`, `tabler-icons.min.css`, `toast.css`, `tokens.css`, `admin.css`, `bootstrap.bundle.min.js` and `toast.js`; they do **not** load `styles.css` (its global `* { margin: 0; padding: 0 }` reset fights Bootstrap). They are theme-aware — see "Dark mode" below. `js/admin-common.js` holds the shared helpers — escaping, `apiFetch` (which rejects with a `.status` so callers can branch on 409/403) and button busy state.
...
- **Link position is load-bearing.** It must come after `bootstrap.min.css` and before the page's own `<style>`, with `tokens.css` immediately before it. `.btn` and `.container` override Bootstrap classes at the *same specificity* and win only on source order — loaded earlier, `.btn-sm` would beat `.btn` and shrink share-links' row buttons, and `.container` would take Bootstrap's 1320px at ≥1400px.
- **Page width and gutter live in `--page-max` / `--page-gutter`, set per page.** Each admin page declares `:root { --page-max: … }` in its own `<style>` block (900/1200/1200/1400) and `admin.css` points `.container`, its padding, and `.footer-buttons` at them. A 1400px table wants width; `access-token` is 900 and has no footer — and it is the *only* one without, despite its stylesheet containing the word `footer-buttons` in a comment. **Four** pages have a footer: housekeeping, access-control and upload at 1200, share-links at 1400. (An earlier revision of this file said three and omitted share-links; the refactor spec's own amendment says four. The measurements below were always taken over four.) `upload` was 800 on the reasoning that a dropzone does not want width, which quietly made its bar 400px narrower than its siblings for no reason a user could see; the page width is the bar width, so aligning the bar meant widening the page. **The bar cannot be widened on its own** — it is sized against the cards, so widening only the bar puts its corners 200px past them on each side, which is the bug `--page-gutter` was added to fix.
...
The admin pages follow the file browser's theme. There is **no toggle on them**: each page's `<head>` runs the same anti-FOUC snippet as `template.html` and `oidc-setup.html` (byte-identical, sets `data-theme` on `document.documentElement` before any stylesheet is fetched), reading the same `localStorage['theme']` the file browser writes and falling back to `prefers-color-scheme`. Flipping the moon in the file browser flips every admin page too, because it is one stored preference on one origin.

**Two token homes.** `css/tokens.css` holds the palette shared with the file browser (`--bg-*`, `--text-*`, `--accent-*`, `--success/warning/danger`, `--border`, `--shadow-*`) and is loaded by *both*; `admin.css` holds the extras only the admin shell needs (`--page-bg`, `--text-subtle`, `--text-on-accent`, `--accent-gradient-*`, `--bg-active`, the `-bg`/`-text`/`-border` triples, `--shadow-accent`). A colour changes in one place.

- **Never move `--radius-*` into `tokens.css`.** `toast.css` asks for `var(--radius-md, 8px)` and that 8px fallback is deliberate — `styles.css` sets 10px, so defining it for the admin pages silently widens every toast. It stays in `styles.css`.
- **The token blocks must be on `:root`, never `body`.** `toast.css` declares `--toast-bg: var(--bg-secondary, #ffffff)` on `:root`, and a custom property's `var()` resolves against the cascade *on the element where it is declared*. On `body` the `:root`-computed `--toast-*` would stay stuck on the fallback and toasts would never follow the theme.
- **`--shadow-sm/md/lg/accent` are whole shorthands, not colours.** `box-shadow: 0 2px 8px var(--shadow-sm)` is malformed and the browser **drops it silently** — no error, no warning, the element just keeps its previous style. Use `box-shadow: var(--shadow-sm)`.

**Bootstrap here is 5.1.3, which predates `data-bs-theme`** (5.3+) and hardcodes its surfaces: `.card`/`.form-control` are `#fff`, `.table` is `#212529`/`#dee2e6`, there is exactly one `var(--bs-body-bg)` in the file, and **zero `--bs-btn-*` variables** — so `--bs-btn-bg` does nothing and every button pseudo-class must be listed. Traps that a colour diff will not show you:

- **Three data-URI SVGs.** `.form-select`'s arrow (`stroke='%23343a40'`) and `.btn-close`'s X (`fill='%23000'`) go invisible on dark and need overriding. `.form-check-input:checked`'s tick (`stroke='%23fff'`) is already correct — which is why that override uses `background-color` only and **never the `background` shorthand**, or it wipes the tick.
- **`.table-hover` paints with an inset `box-shadow`, not a background.** Theming a `td`'s `background-color` does not fix it; all five `--bs-table-*` variables must be overridden. **It only manifests on hover**, so a screenshot of a resting page never reveals it.
- **`.table > :not(:first-child)` is `2px solid currentColor`**, so theming `.table`'s colour gives dark mode a bright rule above every table body unless `border-top-color` is set too.
- **Black shadows are invisible on `#0f172a`**, so card separation in dark has to come from `border-color`.

**The specificity traps.** `[data-theme="dark"] .x` is one class heavier than `.x`, so sweeping with the dark prefix is safe and *painting* with it is not — `[data-theme="dark"] .btn` (0,2,0) would beat `.btn-primary{color:#fff}` (0,1,0) and put dark text on every filled button. Two consequences worth keeping:

- **Bootstrap's outline variants need their `:hover` restated.** `[data-theme="dark"] .btn-outline-primary` and Bootstrap's `.btn-outline-primary:hover` are both (0,2,0); on that tie the override wins the resting colour on source order while the hover rule keeps its own — leaving accent-on-accent text the moment you point at the button. Each variant in `admin.css` therefore sets both states.
- **A bare `.btn` has no variant to override Bootstrap's hardcoded `color: #212529`**, which measured 1.05:1 on a dark card. There is exactly one (`access-control`'s DENY "Add rule"), fixed with `.add-rule:not(.btn-primary)` so the ALLOW button keeps its white label.

**Colours that cannot follow a token.** Inline `style=` attributes beat every class rule, so they had to become classes: `access-control`'s ALLOW/DENY `<h2>`s, and `housekeeping`'s "staged: remove" badge, which three separate JS template strings were emitting with `.alert-danger`'s light palette hardcoded into the string. Grep for the string, not the line.

**Known dead CSS in this family** (defined, never applied — do not assume a rule renders): `share-links`' entire `.stats-card`/`.stats-label`/`.stats-number`/`.status-badge` block (4 of its 11 classes, and the only rules on that page with colours), and `access-token`'s `.btn-copy`. An audit that searches `js/*.js` too, for class names built at runtime, finds everything else in the five pages live.

**`color-scheme: dark` is set on the dark root**, which fixes the overlay scrollbars inside `access-control`'s and `housekeeping`'s scroll cards, `share-links`' bare `<input type="checkbox" class="link-checkbox">` (no Bootstrap class, UA-rendered white), and the `<select>` popup. Use the single keyword `dark`, **never `light dark`** — that means "supports both, pick by `prefers-color-scheme`", which would let the OS override a user who deliberately chose light in the file browser.

**Verifying it.** `tests/tests/admin-theme.spec.ts` asserts the *properties that matter* — that a surface is dark and its text is legible — rather than pinning hex values that drift. Two lessons are baked into it: a luminance threshold **cannot judge `.card-header`** (its light-mode gradient already measures 0.170, below any threshold light chrome satisfies, so it gets a light-vs-dark gradient comparison instead), and every dark sweep carries a **light-mode positive control**, without which a page that failed to load would pass vacuously. The suite drives through `run.sh`, which re-seeds `fileserver/`; `npx playwright test` against a server left over from an earlier run serves stale CSS.
